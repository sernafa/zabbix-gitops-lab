...
NETWORK='zabbix-gitops-lab'
COMPOSE_PROJECT='zabbix-gitops-lab'
CLUSTERS=(central)
...
cluster_name() {
    case "$1" in central) printf 'zabbix-proxy-central' ;; *) fail 'Cliente desconocido.' ;; esac
}
k() {
...
compose() {
    ...
    while IFS= read -r id; do
        [[ -n "$id" ]] || continue
        owner="$(docker inspect "$id" --format '{{index .Config.Labels "zabbix.gitops-lab.owner"}}')"
    ...
    if docker volume inspect "${COMPOSE_PROJECT}_postgres" >/dev/null 2>&1; then
        owner="$(docker volume inspect "${COMPOSE_PROJECT}_postgres" --format '{{index .Labels "zabbix.gitops-lab.owner"}}')"
...
require_network() {
    local owner
    owner="$(docker network inspect "$NETWORK" --format '{{index .Labels "zabbix.gitops-lab.owner"}}')"
...
require_cluster() {
    ...
    owner="$(docker inspect "k3d-$name-server-0" --format '{{index .Config.Labels "zabbix.gitops-lab.owner"}}')"
...
ensure_network() {
    ...
    if [[ ! -f "$STATE_DIR/owner" ]]; then
        if docker network inspect "$NETWORK" >/dev/null 2>&1; then
            recovered_owner="$(docker network inspect "$NETWORK" --format '{{index .Labels "zabbix.gitops-lab.owner"}}')"
            ...
            for endpoint in $(docker network inspect "$NETWORK" --format '{{range $id, $container := .Containers}}{{$id}} {{end}}'); do
                endpoint_owner="$(docker inspect "$endpoint" --format '{{index .Config.Labels "zabbix.gitops-lab.owner"}}')"
    ...
    if docker network inspect "$NETWORK" >/dev/null 2>&1; then
        ...
        docker network create --label "zabbix.gitops-lab.owner=$LAB_ID" "$NETWORK"
    fi
}
